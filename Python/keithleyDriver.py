...
class Keithley2600(Keithley2600Base):
    ...
    def idvgsChar(self,
        ...
        with self._measurement_lock:
            ...
            # self.trigger.blender[2].stimulus[3] = smu3.trigger.MEASURE_COMPLETE_EVENT_ID
            
            # self.trigger.blender[4].orenable = True
            # self.trigger.blender[4].stimulus[1] = smu3.trigger.ARMED_EVENT_ID            #when moved from arm to trigger layer
            # self.trigger.blender[4].stimulus[2] = smu3.trigger.PULSE_COMPLETE_EVENT_ID   # when pulse is complete
            ...
            # smu3.trigger.measure.action = smu3.ENABLE
            # smu3.trigger.measure.stimulus = smu1.trigger.SOURCE_COMPLETE_EVENT_ID
            # smu3.trigger.measure.v(smu3.nvbuffer2)
            ...
            # smu3.trigger.count = step #len(iList)
            ...
            # smu3.trigger.arm.stimulus = self.trigger.EVENT_ID
            # smu3.trigger.arm.count = 1
            ...
            # i_smu2 = self.read_buffer(smu2.nvbuffer1)
            # v_smu3 = self.read_buffer(smu3.nvbuffer2)
            ...
            return i_smu1, v_smu1, v_smu2 #, v_smu3
    ...
    def sourceA_measAB(self,                            # timeseries evaluation
                    ...
                    rangev: float,
                    limitv: float):
        # pico = serial.Serial(com, baudrate=115200)
        with self._measurement_lock:
            timestamp = []
            for smu in [smu1, smu2]:
                smu.source.func = smu.OUTPUT_DCAMPS
    ...
    def doe1Latch(self,                            # DOE 1 amp characterization evaluation
       ...
       with self._measurement_lock:
            ...
            smu2.nvbuffer1.fillmode = smu2.FILL_WINDOW
            smu2.nvbuffer1.fillcount = 1
            ...
            smu2.trigger.source.action = smu2.DISABLE                               # disable channel b source
            smu2.trigger.measure.action = smu2.ASYNC                                # enable smu
            smu2.trigger.measure.i(smu2.nvbuffer1)                                  # measure current and voltage on trigger, store in buffer of smu
            smu2.trigger.measure.stimulus = self.trigger.timer[1].EVENT_ID

            smu1.measure.delay = 0
            smu2.measure.delay = 0
            ...
            while self.status.operation.sweeping.condition > 0:                     # check if sweep has ended
                ...
                i_reading = self.read_buffer(smu2.nvbuffer1)

                if i_reading > expected:                                            # check if current is higher than expected
                    smu1.source.output = smu1.OUTPUT_OFF                            
                    smu2.source.output = smu2.OUTPUT_OFF
                print("Current is: " + i_reading + "Amps")
    ...
    def dualSMU(self,                            # DOE 1 amp characterization evaluation
        ...
        # self.node[2].source.output = smu4.OUTPUT_ON
        # self.tsplink
        self.tsplink.reset()
        time.sleep(2)
        masterNode = self.node[2].tsplink.master
        print('master node: ' + str(masterNode))
        self.tsplink.group = 1
        group = self.node[2].tsplink.group = 1
        ...
        # self.node[2].smub.source.func = smu1.OUTPUT_DCAMPS
        # self.node[2].smua.source.output = smu2.OUTPUT_ON
        # self.node[2].smub.source.output = smu2.OUTPUT_ON
        # self.node[2].smub.measure.autozero = smu2.AUTOZERO_OFF
        return(masterNode)
    
    # def doe2AmpChar(self,                            # DOE 1 amp characterization evaluation
    #                 smu1: KeithleyClass,
    #                 smu2: KeithleyClass,
    #                 vList: Sequence[float],
    #                 delay: float,
    ...
    #     # smu3 = self.node[2].smua
    #     # smu4 = self.node[2].smub
    #     smu2.source.func = smu2.OUTPUT_DCAMPS
    #     smu1.source.func = smu1.OUTPUT_DCVOLTS
    #     for smu in [smu1, smu2]:
    #         smu.source.output = smu.OUTPUT_ON         # SUM 2 is set to measure voltage
    ...
    #     self._write("loadscript CurrentSweep")
    #     self._write("function configNodeB()")
    #     self._write("reset()")
    ...
    #     smu2.source.output = smu2.OUTPUT_ON 
    #     self._write("end")
    #     self._wriite("endscript")
    #     self.write("CurrentSweep.save()")
    #     self.write("CurrentSweep()")
