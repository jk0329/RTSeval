...
void loop()
{
  ...
  switch (command) {
    ...
    case 3: {                                 // iref current sweep Characterization
        digitalWrite(CRSTbin, LOW);           // Low means open circuit
        ...
        waitFor(50);
        for (int j = 129; j >= 1; j--) {      // for loop for the number of columns
          if (colSelect == j) {               // check if j = desired column i.e. 1111...1011
            horSR = 0;                        // if it does set SDA_ to low
          } else {
            horSR = 1;                        // if not set it to high (most cases)
          }
          if (rowSelect == j) {
    ...
    case 5: {                                 // Case for flipping the ctia reset switch during operation
